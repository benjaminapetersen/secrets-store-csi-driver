...
func TestDoesNotFightForeignFields(t *testing.T) {
	...
	g.Expect(got.Labels).To(HaveKeyWithValue("environment", "Iwillreplace"))
	g.Expect(got.Labels).To(HaveKeyWithValue("driverlabel2", "tossed"))
	g.Expect(got.Labels).To(HaveKeyWithValue("foreign.example.com/team", "x"))
	g.Expect(got.Annotations).To(HaveKeyWithValue("kubed.appscode.com/sync", "app=Iwillreplace"))
	g.Expect(got.Annotations).To(HaveKeyWithValue("kubed.appscode.com/sink", "app=tossed"))
	g.Expect(got.Annotations).To(HaveKeyWithValue("foreign.example.com/owned", "yes"))
